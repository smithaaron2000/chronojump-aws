...
// #include </home/aaron/anaconda3/include/python3.8/Python.h>
// #include <stdio.h>
// #include <conio.h>
...
public class ExportSession
{
	...
	private void run_cmd() {
			...
			// string anaconda = "anaconda3/bin/python3.8";
			string python = "/usr/bin/python3.8";
			//string fileName = Path.Combine(home, python);
			// start.FileName = fileName;
			start.FileName = python;
		

			//argument with file name and input parameters
        	string path = "chronojump/src/";
        	string file = "uploadtobucket.py";
        	string fullPath = Path.Combine(home, path, file);

			start.Arguments = string.Format("{0} {1} {2}", Path.Combine(fullPath), 5, 10);
	...
	protected virtual void printData ()
	{
		...
		printJumpsRj(false, Catalog.GetString("Reactive jumps")); //+ 
	...
	protected virtual void printSessionInfo()
	{
		...
		myData.Add ( mySession.UniqueID + ":" + mySession.Name + ":" +
					mySession.Place + ":" + mySession.DateShortAsSQL + ":" + 
	...
	protected void printJumps(string title)
	{
		...
		if(myJumps.Length > 0 ) {
			...
			ArrayList myData = new ArrayList(1);
			myData.Add(
					Catalog.GetString("athlete_id") + ":" +
					Catalog.GetString("athlete_name") + ":" +
					Catalog.GetString("date_time") + ":" +
					Catalog.GetString("jump_id") + ":" + 
					Catalog.GetString("jump_type") + ":" + 
					Catalog.GetString("jump_tc") + ":" + 
					//Catalog.GetString("TF") + ":" + 
					//Catalog.GetString("Fall") + ":" + 
					//weightName + ":" + 
					Catalog.GetString("jump_height") + ":" +
					//Catalog.GetString("Power") + ":" +
					//Catalog.GetString("Stiffness") + ":" +
					//Catalog.GetString("Initial Speed") + ":" +
					Catalog.GetString("jump_RSI")
